...
Progress percentage uses `WalletSummary::progress()` — note-based ratio (notes scanned / total notes), works correctly even when blocks are scanned out of order.
...
runSync():
    ...
    → C callback: sets completedUnitCount = percentage * 10000 (note-based)
...
System provides progress UI (Dynamic Island / Lock Screen) via `task.progress`. Progress uses note-based percentage scaled to 0-10000. Heartbeat fills gaps between batch callbacks.
...
- `startSync()`: calls `rust_sync.startFullSync(mode:1)`, listens to FRB Stream via `.listen()`, fetches balance on each progress event
