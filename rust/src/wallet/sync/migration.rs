...
// denomination-split logic left dead by the 37-part override inside this fn.
...
pub(crate) fn plan_denominations(
    ...
    // Ignore the denomination split and cut `available` into exactly 37 equal parts.
    // With one input, the three split PCZTs plus 37 children fill a 40-message batch.
    // To revert, delete this block and the `#[allow(unreachable_code)]` on the fn.
    #[cfg(not(test))]
    {
        const TEST_FORCED_PARTS: u64 = 37;
        ...
        if per_part < per_part_floor {
            return Err(format!(
                "37-part test override: available {} / {} = {} below per-part floor {} \
                 (migration_fee + minimum_output); fund more Orchard balance to test 37 parts",
